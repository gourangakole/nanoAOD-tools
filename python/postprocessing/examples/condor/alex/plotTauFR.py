...
def getListOfFiles(dataset="Run2016B-DoubleMuon-NANOAOD-02Apr2020"):
    ...
    '''
    basedir   = "/afs/cern.ch/user/a/attikis/workspace/nanoAOD/"
    fileLists = {}
    fileLists["Run2016B-DoubleMuon-NANOAOD-02Apr2020"] = ["%sC4804A85-5EE9-ED4A-8F87-FE23D9A682AE_Skim.root" % (basedir),
                                                          "%sAACB26B2-3924-3F48-9E27-30FAD0AD092F_Skim.root" % (basedir),
                                                          "%s9F890C30-18E8-2D42-920F-5BDE836E3B57_Skim.root" % (basedir),
                                                          "%s96200C93-DA5C-C145-A0E8-FB59D7E584A0_Skim.root" % (basedir),
                                                          "%s7985E7AF-EC8F-C34D-9848-44A5E3EDE0A2_Skim.root" % (basedir),
                                                          "%s792E70B0-2715-F24B-A8DE-1327E9F6480B_Skim.root" % (basedir),
                                                          "%s3D4A9ACB-7ACC-AA4B-A3F1-F300B50D6196_Skim.root" % (basedir),
                                                          "%s13CBB9A0-5CCA-0347-8C64-1F0A7E652071_Skim.root" % (basedir)]
...
class ExampleAnalysis(Module):
    ...
    def analyze(self, event):
        ...
        jetsToClean = []
        nTightMuons = 0
        muon_p4     = [] #ROOT.TLorentzVector()
        
        for mu in muons:
            if (mu.mvaId>=1): #Mva ID from miniAOD selector (1=MvaLoose, 2=MvaMedium, 3=MvaTight)
                jetsToClean.append(mu.jetIdx)
                if (mu.mvaId==3):
                    nTightMuons+= 1            
                    muon_p4.append(mu.p4())
                    #muon_p4 += mu.p4()

        if nTightMuons != 2:
            return
        if (muon_p4[0] + muon_p4[1]).M() >  91.1876 + 20:
            return
        if (muon_p4[0] + muon_p4[1]).M() < 91.1876 - 20:
            return

        # https://root.cern.ch/doc/master/classTLorentzVector.html
        self.hDimuonPt.Fill( (muon_p4[0] + muon_p4[1]).Pt() )
        self.hDimuonMass.Fill( (muon_p4[0] + muon_p4[1]).M() )
        self.hDimuonDeltaEta.Fill(abs(muon_p4[0].Eta() - muon_p4[1].Eta()))
        self.hDimuonDeltaPhi.Fill(muon_p4[0].DeltaPhi(muon_p4[1]))
        self.hDimuonDeltaR.Fill(muon_p4[0].DeltaR(muon_p4[1]))

        # For-loop: All taus
        for tau in taus:

            # Cleaning (https://hypernews.cern.ch/HyperNews/CMS/get/physTools/3729/1.html)
            #if tau.jetIdx in jetsToClean:
                # continue

            # Retrieve discriminators
            ...
            if (bPassVSjetL):
                ...
                # print "tau.cleanmask = %d, tau.jetIdx = %d" % (tau.cleanmask, tau.jetIdx)
	...
	# Events with at least 1 loose tau
        ...
        self.hTauDM_LooseTau.Fill(looseTaus[0].decayMode)
        if looseTaus[0].jetIdx > -1:
            self.hTauPtJetPtRatio_LooseTau.Fill(looseTaus[0].p4().Pt() / jets[looseTaus[0].jetIdx].p4().Pt())
	...
	# Events with at least 1 tight tau
        ...
        muonJetPtRelv2_min = -1.0
        muonJetRelIso_min  = -1.0
        muonPFRelIso03_min = -1.0

        # For-loop: All muons
        for mu in muons:
            #if not (mu.mvaId>=1): #Mva ID from miniAOD selector (1=MvaLoose, 2=MvaMedium, 3=MvaTight)
            #    continue #leto
            ...
            if dR < dR_min:
                dR_min  = dR
                dPt_min = (tau_p4.Pt() - mu.p4().Pt())/tau_p4.Pt()
                muonPt_min  = mu.p4().Pt()
        ...
        if tightTaus[0].jetIdx > -1:
            self.hTauPtJetPtRatio_TightTau.Fill(tightTaus[0].p4().Pt() / jets[tightTaus[0].jetIdx].p4().Pt())
        #
